 ...
 * where it's redundant. WhatsApp is the single conversion path (owner,
 * 2026-08 — the "book a call" button was retired from every surface).
 */
export default function MobileStickyCTA() {
  const [visible, setVisible] = useState(false);
  const path = usePathname() ?? "/";
  const isAr = localeFromPath(path) === "ar";
  const hide = path.startsWith("/admin") || /^\/((fr|ar|en)\/)?(appel|contact)/.test(path);
